...
export class SettingsManager {
  ...
  public updateSetting<K extends keyof UserSettings>(
    ...
    console.log(`⚙️ ${LanguageManager.t('console.settingsUpdated')}: ${category}`, updates);
  }
  ...
  public resetToDefaults(): void {
    ...
    saveSettings(this.settings);
    this.notifyListeners();
    console.log('🔄 ' + LanguageManager.t('console.settingsResetToDefaults'));
  }
  ...
  private exportData(): void {
    try {
      ...
      console.log('📤 ' + LanguageManager.t('console.dataExportedSuccessfully'));
    } catch (error) {
      ...
      reader.onload = (e) => {
        try {
          ...
          if (importUserData(data)) {
            ...
            console.log('📥 ' + LanguageManager.t('console.dataImportedSuccessfully'));
          }
        } catch (error) {
  ...
  private confirmReset(): void {
    if (confirm('🗑️ Tüm verileri sıfırlamak istediğinizden emin misiniz? Bu işlem geri alınamaz!')) {
      ...
      console.log('🔄 ' + LanguageManager.t('console.allDataReset'));
    }
  }
...
export class AchievementsManager {
  ...
  public renderAchievements(): void {
    ...
    container.innerHTML = filteredAchievements.map(achievement => {
      ...
      const title = i18nInstance.t(achievement.titleKey);
      const description = i18nInstance.t(achievement.descriptionKey);
      const categoryName = i18nInstance.t(category.nameKey);
      const rarityName = i18nInstance.t(rarity.nameKey);
      const pointsText = i18nInstance.t('achievements.points');
      const progressText = i18nInstance.t('achievements.progress');
      ...
      return `
        <div class="achievement-card ${isUnlocked ? 'unlocked' : 'locked'}">
          ...
          <div class="achievement-category ${achievement.category}" style="color: ${category.color}">
            ${categoryName}
          </div>
          
          <div class="achievement-rarity ${achievement.rarity}" style="color: ${rarity.color}">
            ${rarityName}
          </div>
          
          <div class="achievement-reward">
            +${achievement.reward?.value || 0} ${pointsText}
          </div>
          ...
          ${!isUnlocked ? `
            <div class="achievement-progress">
              ...
              <div class="progress-text">${progress}% ${progressText}</div>
            </div>
          ` : ''}
        </div>
      `;
  ...
  private unlockAchievement(achievementId: string): void {
    ...
    if (achievement) {
      const title = i18nInstance.t(achievement.titleKey);
      console.log(`🏆 ${LanguageManager.t('console.achievementUnlockedNotification')}: ${title}`);
  ...
  private showAchievementNotification(achievement: any): void {
    ...
    const achievementUnlockedText = i18nInstance.t('achievements.achievementUnlocked');
    const achievementTitle = i18nInstance.t(achievement.titleKey);
    
    notification.innerHTML = `
      <div style="display: flex; align-items: center; gap: 10px;">
        <span style="font-size: 20px;">${achievement.icon}</span>
        <div>
          <div style="font-weight: 600;">${achievementUnlockedText}</div>
