...
export class AdMobService {
  ...
  private async initialize(): Promise<void> {
    try {
      if (!Capacitor.isNativePlatform()) {
        console.log('📱 AdMob: Web platformda çalışmıyor, native gerekli');
        return;
      }
      ...
      console.log('🎯 AdMob başarıyla başlatıldı', { testMode: this.isTestMode });
  ...
  public async showBannerAd(position: 'top' | 'bottom' = 'bottom'): Promise<boolean> {
    ...
    try {
      ...
      console.log('🎯 Banner reklam gösterildi:', { position, testMode: this.isTestMode });
      return true;
  ...
  public async prepareInterstitialAd(): Promise<boolean> {
    ...
    try {
      ...
      console.log('🎯 Interstitial reklam hazırlandı:', { testMode: this.isTestMode });
      return true;
  ...
  public async showGameOverAd(): Promise<boolean> {
    console.log('🎮 Oyun bitişi reklamı gösteriliyor...');
  ...
  public async showLevelCompleteAd(level: number): Promise<boolean> {
    ...
    if (level % 3 === 0) {
      console.log(`🎮 Seviye ${level} tamamlandı, reklam gösteriliyor...`);
      return await this.showInterstitialAd();
    }
    return false;
  }
  ...
  public async showMenuBannerAd(): Promise<boolean> {
    console.log('🎮 Ana menü banner reklamı gösteriliyor...');
    return await this.showBannerAd('bottom');
  }

  public async hideMenuBannerAd(): Promise<boolean> {
    console.log('🎮 Ana menü banner reklamı gizleniyor...');
