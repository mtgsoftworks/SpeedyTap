...
import { generateRandomNumber, generateRandomPosition, getElementSize, addCSSAnimation, vibrate, adMobService, AchievementsManager, DeltaTimer, GameLoop, PerformanceMonitor, WebPerformance } from '@/utils';
...
export class GameScreenManager {
  ...
  public async initialize(): Promise<void> {
    console.log('🎮 Game Screen Manager başlatılıyor...');
    ...
    try {
      ...
      console.log('🔊 Audio Manager hazır');
      ...
      console.log('✅ Game Screen Manager hazır!');
  ...
  private initializeWebPerformance(): void {
    ...
    console.log(`📱 Cihaz performansı: ${isHighPerf ? 'Yüksek' : 'Standart'}`);
    
    // Enable performance monitor in debug mode
    if (APP_CONFIG.PERFORMANCE.ENABLE_PERFORMANCE_MONITOR) {
      this.performanceMonitor.show();
      console.log('📊 Performance Monitor etkinleştirildi');
    }
    
    // Start optimized game loop
    this.gameLoop.start();
    console.log('🎮 Optimized Game Loop başlatıldı');
  }
  ...
  private async initializeAds(): Promise<void> {
    try {
      ...
      console.log('🎯 AdMob reklamları hazırlandı');
    } catch (error) {
  ...
  public startGameFromTutorial(): void {
    console.log('🚀 Oyun tutorial\'dan başlatılıyor...');
    ...
    try {
      ...
      console.log(`📄 Mevcut sayfa: ${PAGES.PLAY_DELAY}'e geçiliyor`);
      
      // Show countdown
      this.showPage(PAGES.PLAY_DELAY);
      
      console.log('⏱️ Countdown başlatılıyor...');
      this.startCountdown();
  ...
  private startCountdown(): void {
    ...
    console.log('🎯 Countdown elementi bulundu, başlatılıyor...');
    ...
    const countdownInterval = setInterval(() => {
      if (count > 1) {
        ...
        console.log('🎮 Countdown tamamlandı, oyun alanı açılıyor...');
  ...
  private startLevel(): void {
    console.log(`🎮 Seviye ${this.gameState.level} başlatılıyor...`);
  ...
  private handleEvilCircleTap(): void {
    console.log('💀 Kötü daireye dokunuldu!');
    ...
    this.gameOver('Kaybettin').catch(console.error);
  }
  ...
  private startTimer(): void {
    ...
    this.gameTimer = new DeltaTimer(
      ...
      // onComplete callback
      () => {
        this.gameOver("Süre Doldu").catch(console.error);
      }
    );
